...
//	}
    ...
    public Double getMaxDepthInMeters() {
        Double rawMax = getMaxdepth();
        Units.Distance units = getMaxdepthUnitTyped();
        if (rawMax == null) {
            return null;
        }
        return units == Units.Distance.KM ? rawMax : Converter.convert(rawMax, Units.Distance.FT, Units.Distance.KM);
    }
    ...
    public Units.Distance getMaxdepthUnitTyped() {
        String rawUnits = getMaxdepthUnit();
        if (rawUnits == null) {
            //prefer metric
    ...
    public Double getTempBottomCelcius() {
        Double rawTemp = getTempBottom();
        Units.Temperature units = getTempBottomUnitTyped();
        if (rawTemp == null) {
            return null;
        }
        return units == Units.Temperature.C ? rawTemp : Converter.convert(rawTemp, Units.Temperature.F, Units.Temperature.C);
    }

    public Units.Temperature getTempBottomUnitTyped() {
        String rawUnits = getTempBottomUnit();
        if (rawUnits == null) {
            //prefer metric
            return Units.Temperature.C;
        }
        return (rawUnits.compareTo("C") == 0) ? Units.Temperature.C : Units.Temperature.F;
    }
