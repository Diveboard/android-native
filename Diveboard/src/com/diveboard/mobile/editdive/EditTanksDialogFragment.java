...
public class					EditTanksDialogFragment extends DialogFragment
{
	...
	private int						mTextSize = 20;
	 ...
	 public void onAttach(Activity activity)
	 {
		 ...
		 catch (ClassCastException e)
		 {
			 ...
			 throw new ClassCastException(activity.toString() + " must implement onTanksEditComplete");
		 }
	 }
	...
	private void				openTanksEdit(final Integer index)
	{
		if(index == -1)
			mIndex = mTanks.size() - 1;
		else
			mIndex = index;
		LinearLayout tankslist = (LinearLayout) mView.findViewById(R.id.tanksfields);
		tankslist.removeAllViews();
		final float scale = getResources().getDisplayMetrics().density;
		final Tank tank = mTanks.get(mIndex);
...
//		ArrayAdapter<String> cylinderAdapter = new ArrayAdapter<String>(getActivity().getApplicationContext(), R.layout.units_spinner);
		ArrayAdapter<String> cylinderAdapter = new DiveboardSpinnerAdapter(getActivity().getApplicationContext(), R.layout.units_spinner);
		...
		ArrayAdapter<String> materialAdapter = new DiveboardSpinnerAdapter(getActivity().getApplicationContext(), R.layout.units_spinner);
		...
		ArrayAdapter<String> volumeAdapter = new DiveboardSpinnerAdapter(getActivity().getApplicationContext(), R.layout.units_spinner);
		...
		ArrayAdapter<String> adapt_mix = new DiveboardSpinnerAdapter(getActivity().getApplicationContext(), R.layout.units_spinner);
		...
		ArrayAdapter<String> press_adapt = new DiveboardSpinnerAdapter(getActivity().getApplicationContext(), R.layout.units_spinner);
	...
	private void				openTanksList()
	{
		...
		add_button.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View v)
			{
				//we create a default tank and pass it to edit it
				try{
					JSONObject jTank = new JSONObject();
					jTank.put("material", "aluminium");
					jTank.put("multitank", 1);
					jTank.put("volume_value", 12.0);
					jTank.put("volume_unit", "L");
					jTank.put("o2", 21);
					jTank.put("n2", 79);
					jTank.put("he", 0);
					jTank.put("gas_type","air");
					jTank.put("time_start", 0);
					if(Units.getPressureUnit() == Units.Pressure.BAR){
						jTank.put("p_start_value", 200.0);
						jTank.put("p_end_value", 50.0);
						jTank.put("p_start_unit", "bar");
						jTank.put("p_end_unit", "bar");
					}else{
						jTank.put("p_start_value", 3000.0);
						jTank.put("p_end_value", 500.0);
						jTank.put("p_start_unit", "psi");
						jTank.put("p_end_unit", "psi");
					}
					
					Tank newTank = new Tank(jTank);
					mTanks.add(newTank);
					//new tank index is -1
					openTanksEdit(-1);
